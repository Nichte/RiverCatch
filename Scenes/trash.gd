...
func _physics_process(delta):
	...
	if position.x < net_position_x + 10  and not has_scored:
		...
		if is_small == true:
			emit_signal("missed_trash")
		else:
			emit_signal("scored_negative")  # Notify main.gd
			queue_free()
		
	if position.x < -50:
		queue_free()

func _on_trash_area_input_event(_viewport: Node, event: InputEvent, _shape_idx: int) -> void:
